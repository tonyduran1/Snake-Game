...
class Polygon {
  ...
  public boolean collides(Polygon other) {
      ...
      if (thisPoints.length == 0 || otherPoints.length == 0) {
          // One of the polygons is not correctly initialized.
          return false;
      }

      // Check if any point of this polygon is contained within the other polygon.
      for (int i = 0 ; i < thisPoints.length; i++) {
          if (other.contains(thisPoints[i])) {
              return true;  
          }
      }

      // Check if any point of the other polygon is contained within this polygon.
